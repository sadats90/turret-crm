...
class Home_Details_Report extends Model
{
    public function __construct() {
        $this->company_code = session('the_company_code') ?? '';
        $this->emp_code = session('the_emp_code') ?? '';
    }

    public function details_report($place_type, $period_type, $product_type, $stores, $from_date, $to_date, $articles) {
        ...
        $data = DB::table('calendars')
            ->select('calendars.PeriodId as week_in', 
                //DB::raw('0 as total_est'), 
                DB::raw('SUM(ifnull(location_estimates.sale_price, 0)) as total_est'),
                DB::raw('0 as fw_est'), DB::raw('0 as nfw_est'),
                DB::raw('sum(sales_msts.Total_Qty) as tot_qty'),
                DB::raw('sum(sales_msts.total_fw) as tot_fw'),
                DB::raw('sum(sales_msts.total_nfw) as tot_nfw'),
                DB::raw('0 as total_est_value'), 
                DB::raw('0 as fw_est_value'), DB::raw('0 as nfw_est_value'),
                DB::raw('COUNT(sales_msts.id) as tot_trx'),
                DB::raw('sum(sales_msts.Total_Value) as tot_value'),
                DB::raw('sum(sales_msts.total_discount_amount) as tot_discount'),
                DB::raw('sum(sales_msts.total_fw_value) as tot_fw_value'),
                DB::raw('sum(sales_msts.total_nfw_value) as tot_nfw_value'),
                DB::raw('MIN(sales_msts.created_at) as min_created_at'),
                DB::raw('MAX(sales_msts.created_at) as max_created_at')
                //DB::raw("DATE_FORMAT(sales_msts.created_at, '%Y-%m-%d') as created_date")
            )->join('sales_msts', function($join) {
                $join->on('calendars.Company_Code', '=', 'sales_msts.Company_Code')
                    ->where(DB::raw("DATE_FORMAT(calendars.StartDate, '%m/%d/%Y')"), '<=', DB::raw("DATE_FORMAT(sales_msts.created_at, '%m/%d/%Y')"))
                    ->where(DB::raw("DATE_FORMAT(calendars.EndDate, '%m/%d/%Y')"), '>=', DB::raw("DATE_FORMAT(sales_msts.created_at, '%m/%d/%Y')"));

                    return $join;
                }
            )->leftJoin('location_estimates', function($left_join) use($stores, $from_date, $to_date){
                $left_join
                    ->on('location_estimates.Company_Code', '=', 'calendars.Company_Code')
                    ->where('location_estimates.period_type', '=', 'Day')
                    ->where('location_estimates.period_id', '>=', '2019-03-04')
                    ->where('location_estimates.period_id', '<=', '2019-03-10')
                    ->where('location_estimates.group_type', '=', 'Store')
                    ->whereIn('location_estimates.group_id', $stores);
                    return $left_join;
                }
            )
            ->where('sales_msts.created_at', '>=', $from_date)
            ->where('sales_msts.created_at', '<=', $to_date)
            ->where('calendars.PeriodType', $period_type)

            /*->where('location_estimates.period_type', '=', 'Day')
            ->where('location_estimates.period_id', '>=', 'calendars.StartDate')
            ->where('location_estimates.period_id', '<=', 'calendars.EndDate')
            ->where('location_estimates.group_type', '=', 'Store')
            ->whereIn('location_estimates.group_id', $stores)*/

            ->where(function($query) use($stores, $articles) {
                    if (count($stores) > 0) {
                        $query->whereIn('sales_msts.Store_Code', $stores);
                    }
                    if (count($articles) > 0) {
                        //$query->whereIn('sales_msts.Article_mstID', $articles);
                    }
                    return $query;
                }
            )->groupBy('calendars.PeriodId')
            ->orderBy('calendars.PeriodId')
            ->get();

        /*if ($data->count() > 0) {
            $data = $data->toArray();
            foreach ($data as $key => $item) {
                $item->total_est = DB::table('location_estimates')
                    ->where('Company_Code', $this->company_code)
                    ->where('delete_cd', 'Y')
                    ->whereBetween('period_id', [$item->min_created_at, $item->max_created_at])
                    ->select(DB::raw('SUM(sale_price) as est_sum'))
                    ->get()->first()->est_sum;
            }
        }*/
